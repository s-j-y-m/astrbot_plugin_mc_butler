...
**绑定数据以本插件为权威源**：任何变更入口（QQ 指令 / LLM 工具 / WebUI）都先写本地 KV，再全量同步到 Velocity；Velocity 侧只读，不做本地编辑。两个配套组件开源在 [astrbot_plugin_mc_butler_VC-MCDR](https://github.com/s-j-y-m/astrbot_plugin_mc_butler_VC-MCDR) 仓库。

## 前置要求

- AstrBot >= v4.10.4（本插件使用 `PluginKVStoreMixin`，即 `Star` 基类提供的 KV 存储）
- Velocity 代理已部署 [mclink-velocity](https://github.com/s-j-y-m/astrbot_plugin_mc_butler_VC-MCDR/tree/main/mclink-velocity) 插件，HTTP 桥监听 `127.0.0.1:28080`
- 各子服 MCDR 安装 [rcon_bridge](https://github.com/s-j-y-m/astrbot_plugin_mc_butler_VC-MCDR/tree/main/rcon_bridge) 插件（在 MCDR 内起 RCON **服务端**，生存服监听 `127.0.0.1:30166`、镜像服 `30167`），端口/密码与 Velocity 插件 `config.yml` 的 `servers.*.rcon-*` 一致
